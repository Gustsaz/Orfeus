...
function switchTab(tabId) {
  const currentActivePage = document.querySelector('.tab-page.active');
  ...
  // Se estávamos no afinador e saímos, limpar display e parar microfone
  if (currentActiveId === 'afinador' && tabId !== 'afinador') {
    if (typeof tunerNote !== 'undefined' && tunerNote) tunerNote.textContent = '--';
    if (typeof markers !== 'undefined' && markers && markers.length) markers.forEach(m => m.style.background = '#555');
    if (typeof running !== 'undefined' && running) {
      if (typeof stopMic === 'function') stopMic();
      if (typeof micIcon !== 'undefined' && micIcon) micIcon.src = 'imgs/mic_off_icon.png';
    }
  }
}
...
function resetTuner() {
  // Mantém última nota reconhecida até sair da aba/recarregar
...
function stopMic() {
  ...
  // não resetar a nota aqui – manter última nota
}
...
let sampler = null;
let drumEngines = null; // motores de som para a bateria
...
async function switchInstrument(inst) {
  ...
  if (inst === 'piano') {
    ...
    // Substitui por síntese para evitar 404 de samples externos
    sampler = new Tone.PluckSynth().toDestination();
  }
  else if (inst === 'drums') {
    // Bateria baseada em sintetizadores do Tone.js (sem amostras externas)
    if (sampler) { sampler.dispose(); sampler = null; }
    if (drumEngines) {
      // limpar engines anteriores
      Object.values(drumEngines).forEach(s => s.dispose && s.dispose());
    }
    drumEngines = {
      kick: new Tone.MembraneSynth({ pitchDecay: 0.02, octaves: 6, oscillator: { type: 'sine' }, envelope: { attack: 0.001, decay: 0.4, sustain: 0.0, release: 1.4 } }).toDestination(),
      snare: new Tone.NoiseSynth({ noise: { type: 'white' }, envelope: { attack: 0.001, decay: 0.2, sustain: 0 } }).toDestination(),
      hihat: new Tone.NoiseSynth({ noise: { type: 'white' }, envelope: { attack: 0.001, decay: 0.05, sustain: 0 } }).toDestination(),
      tom1: new Tone.MembraneSynth({ octaves: 4 }).toDestination(),
      tom2: new Tone.MembraneSynth({ octaves: 4 }).toDestination(),
      tom3: new Tone.MembraneSynth({ octaves: 4 }).toDestination(),
      crash: new Tone.MetalSynth({ frequency: 200, envelope: { attack: 0.001, decay: 0.4, release: 0.2 }, harmonicity: 5.1, modulationIndex: 32, resonance: 4000, octaves: 1.5 }).toDestination(),
      ride: new Tone.MetalSynth({ frequency: 300, envelope: { attack: 0.001, decay: 1.0, release: 0.4 }, harmonicity: 5.1, modulationIndex: 12, resonance: 6000, octaves: 1.5 }).toDestination()
    };
  } else if (inst === 'flute') {
    // Síntese de flauta (onda senoidal com leve ataque)
...
function renderInstrumentUI(inst) {
  ...
  if (inst === 'flute') {
    ...
    title.textContent = 'Flauta (toque os orifícios)';
    ...
    svg.setAttribute('viewBox', '0 0 820 140');
    svg.setAttribute('width', '95%');
    svg.setAttribute('height', '160');

    // Corpo da flauta
    const body = document.createElementNS(svgNS, 'rect');
    body.setAttribute('x', '20'); body.setAttribute('y', '50');
    body.setAttribute('width', '780'); body.setAttribute('height', '40');
    body.setAttribute('rx', '20');
    body.setAttribute('fill', '#b0b0b0');
    ...
    letters.forEach((letter, idx) => {
      ...
      circle.setAttribute('cx', String(startX + idx * gap));
      ...
      text.setAttribute('x', String(startX + idx * gap));
      text.setAttribute('y', String(cy + 35));
      text.setAttribute('fill', '#fff');
      text.setAttribute('font-size', '16');
      text.setAttribute('text-anchor', 'middle');
      text.textContent = letter; // sem oitava
      ...
      group.addEventListener('mousedown', () => {
        ...
        playNote(noteMap[letter]);
      });
      group.addEventListener('mouseup', () => {
        circle.setAttribute('fill', '#222');
        stopNote(noteMap[letter]);
      });
    ...
    instrumentArea.appendChild(wrapper);
  }
  ...
  if (inst === 'drums') {
    ...
    // SVG simples de um kit de bateria
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS, 'svg');
    ...
    function circle(cx, cy, r, fill, id, label) {
      const g = document.createElementNS(svgNS, 'g');
      g.setAttribute('id', id);
      const c = document.createElementNS(svgNS, 'circle');
      c.setAttribute('cx', cx); c.setAttribute('cy', cy); c.setAttribute('r', r);
      c.setAttribute('fill', fill); c.setAttribute('stroke', '#222'); c.setAttribute('stroke-width', '4');
      c.style.cursor = 'pointer';
      const text = document.createElementNS(svgNS, 'text');
      text.setAttribute('x', cx); text.setAttribute('y', cy + 6);
      text.setAttribute('fill', '#fff'); text.setAttribute('font-size', '18'); text.setAttribute('text-anchor', 'middle');
    ...
    function ellipse(cx, cy, rx, ry, fill, id, label) {
      const g = document.createElementNS(svgNS, 'g');
      g.setAttribute('id', id);
      const e = document.createElementNS(svgNS, 'ellipse');
      e.setAttribute('cx', cx); e.setAttribute('cy', cy); e.setAttribute('rx', rx); e.setAttribute('ry', ry);
      e.setAttribute('fill', fill); e.setAttribute('stroke', '#222'); e.setAttribute('stroke-width', '4');
      e.style.cursor = 'pointer';
      const text = document.createElementNS(svgNS, 'text');
      text.setAttribute('x', cx); text.setAttribute('y', cy + 6);
      text.setAttribute('fill', '#fff'); text.setAttribute('font-size', '18'); text.setAttribute('text-anchor', 'middle');
      text.textContent = label;
      g.appendChild(e); g.appendChild(text);
      svg.appendChild(g);
      return g;
    }

    // Peças
    const kick = circle(400, 340, 90, '#2a2a2a', 'kick', 'Kick');
    const snare = circle(260, 320, 60, '#3a3a3a', 'snare', 'Snare');
    const hihat = ellipse(220, 220, 70, 18, '#555500', 'hihat', 'Hi-Hat');
    const tom1 = circle(360, 240, 45, '#3a3a3a', 'tom1', 'Tom 1');
    const tom2 = circle(440, 240, 50, '#3a3a3a', 'tom2', 'Tom 2');
    const tom3 = circle(520, 300, 60, '#3a3a3a', 'tom3', 'Floor');
    const crash = ellipse(560, 180, 85, 22, '#6b6b00', 'crash', 'Crash');
    const ride = ellipse(640, 220, 80, 20, '#6b6b00', 'ride', 'Ride');
    ...
    function playDrum(part) {
      if (!drumEngines) return;
      switch (part) {
        case 'kick': drumEngines.kick.triggerAttackRelease('C2', '8n'); break;
        case 'snare': drumEngines.snare.triggerAttackRelease('8n'); break;
        case 'hihat': drumEngines.hihat.triggerAttackRelease('16n'); break;
        case 'tom1': drumEngines.tom1.triggerAttackRelease('E3', '8n'); break;
        case 'tom2': drumEngines.tom2.triggerAttackRelease('D3', '8n'); break;
        case 'tom3': drumEngines.tom3.triggerAttackRelease('B2', '8n'); break;
        case 'crash': drumEngines.crash.triggerAttackRelease('8n'); break;
        case 'ride': drumEngines.ride.triggerAttackRelease('8n'); break;
      }
    }

    // listeners
    [kick, snare, hihat, tom1, tom2, tom3, crash, ride].forEach(g => {
...
function buildKeyboard() {
  ...
  for (let octave = startOctave; octave <= endOctave; octave++) {
    whiteOrder.forEach((note) => {
      ...
      // Para flauta, mostrar apenas a letra da nota
      label.textContent = (currentInstrument === 'flute') ? `${note}` : `${note}${octave}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Event listener para mudança de abas (remove referências antigas)
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      // sem inicialização de virtualPiano
    });
  });
...
function openActivity(id) {
  ...
  if (id === "agudoGrave") {
    ...
    inner.innerHTML = `
      <h2>Atividade: Duração — Longo ou Curto</h2>
      <p>Um som será tocado com duração curta ou longa. Escolha a opção correta.</p>
      <div style="margin-top:20px;">
        <button onclick="checkDuracao('curto')" class="ans-btn">Curto</button>
        <button onclick="checkDuracao('longo')" class="ans-btn">Longo</button>
      </div>
      <p id="duracaoFeedback" style="margin-top:15px;"></p>
    `;
    playDuracao();
  }
  else if (id === 'intensidade') {
    inner.innerHTML = `
      <h2>Atividade: Intensidade — Forte ou Fraco</h2>
      <p>Um som será tocado com intensidade forte (forte) ou fraca (piano). Escolha corretamente.</p>
      <div style="margin-top:20px;">
        <button onclick="checkIntensidade('fraco')" class="ans-btn">Fraco</button>
        <button onclick="checkIntensidade('forte')" class="ans-btn">Forte</button>
      </div>
      <p id="intensidadeFeedback" style="margin-top:15px;"></p>
    `;
    playIntensidade();
  }
...
let currentDuracao = null;
function playDuracao() {
  const synth = new Tone.Synth().toDestination();
  const note = 'C4';
  if (Math.random() > 0.5) {
    currentDuracao = 'curto';
    synth.triggerAttackRelease(note, '8n');
  } else {
    currentDuracao = 'longo';
    synth.triggerAttackRelease(note, '2n');
  }
}

async function checkDuracao(ans) {
  const el = document.getElementById('duracaoFeedback');
  if (ans === currentDuracao) {
    el.textContent = '✅ Correto! +10 pontos';
    el.style.color = 'lightgreen';
    await addUserPoints(10);
    setTimeout(() => { el.textContent = ''; playDuracao(); }, 1500);
  } else {
    el.textContent = '❌ Errado! Tente novamente.';
    el.style.color = 'red';
  }
}

// ================== INTENSIDADE (FORTE x FRACO) ==================
let currentIntensidade = null;
function playIntensidade() {
  const synth = new Tone.Synth().toDestination();
  const note = 'E4';
  if (Math.random() > 0.5) {
    currentIntensidade = 'forte';
    synth.volume.value = 0; // mais alto
    synth.triggerAttackRelease(note, '4n');
  } else {
    currentIntensidade = 'fraco';
    synth.volume.value = -18; // mais baixo
    synth.triggerAttackRelease(note, '4n');
  }
  // reset volume após breve tempo
  setTimeout(() => { synth.volume.value = 0; }, 800);
}

async function checkIntensidade(ans) {
  const el = document.getElementById('intensidadeFeedback');
  if (ans === currentIntensidade) {
    el.textContent = '✅ Correto! +10 pontos';
    el.style.color = 'lightgreen';
    await addUserPoints(10);
    setTimeout(() => { el.textContent = ''; playIntensidade(); }, 1500);
  } else {
    el.textContent = '❌ Errado! Tente novamente.';
    el.style.color = 'red';
  }
...
function showInstallButton() {
  ...
  installBtn.className = 'edit-profile-btn';
